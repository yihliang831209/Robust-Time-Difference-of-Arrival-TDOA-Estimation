...
class Encoder_TorchSTFT(nn.Module):
    ...
    """
    def __init__(self, n_fft_short, n_hop, center=False, window=None):
        super(Encoder_TorchSTFT, self).__init__()

        self.n_fft_short = n_fft_short
        self.n_hop = n_hop
        self.center = center
        self.window = nn.Parameter(torch.hamming_window(n_fft_short), requires_grad=False)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        complex_stft_short = torch.stft(x, n_fft=self.n_fft_short, hop_length=self.n_hop, window=self.window, center=self.center,\
        ...
        # print('shape.encoder_stft_long:'+str(complex_stft_long.shape))
        
        # stft = torch.cat([complex_stft_short, complex_stft_long],dim=1)
        
        # print('shape.encoder:'+str(stft.shape))
        stft_f = torch.view_as_real(complex_stft_short)  #abs,phase值 [batch, 33, 499, 2]       
...
class Decoder_TorchISTFT(nn.Module):
    ...
    def __init__(self,n_fft_short, n_hop, center: bool = False, sample_rate: float = 16000.0,window=None):
...
class OpenUnmix(nn.Module):
    ...
    def __init__(
        ...
        self.fc3 = Linear(
            ...
            out_features=self.nb_output_bins * nb_output_channels,
            bias=False,
        )

        self.bn3 = BatchNorm1d(self.nb_output_bins * nb_output_channels)
    ...
    def forward(self, x: Tensor,hn,cn) -> Tensor:
        """
        Args:
        ...
        x = x.permute(3, 0, 1, 2) #(nb_frames,nb_samples, nb_channels, nb_bins )
        ...
        x = x.reshape(nb_frames, nb_samples, self.nb_output_channels*self.nb_output_bins)
        ...
        #(nb_frames, nb_samples, nb_channels, nb_output_bins)
        # print('mix_shape:'+str(mix.shape))
        # print('x_shape:'+str(x.shape))
        x = F.sigmoid(x)
        # permute back to (nb_samples, nb_channels, nb_bins, nb_frames)
        return x.permute(1, 2, 3, 0),lstm_out[1]
